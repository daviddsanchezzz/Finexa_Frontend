...
import FlightDetailsFields from "./components/FlightDetailsFields";
import AccommodationDetailsFields from "./components/AccommodationDetailsFields";
import TransportDetailsFields from "./components/TransportDetailsFields";
...
export default function TripPlanFormScreen({
  ...
  const [flightProvider, setFlightProvider] = useState<FlightProvider | null>(
    (planItem as any)?.flightDetails?.provider || null
  );
  const [flightAirlineName, setFlightAirlineName] = useState<string>(
    (planItem as any)?.flightDetails?.airlineName || ""
  );
  const [flightNumber, setFlightNumber] = useState<string>(
    (planItem as any)?.flightDetails?.flightNumberRaw || ""
  );
  const [flightFromIata, setFlightFromIata] = useState<string>(
    (planItem as any)?.flightDetails?.fromIata || ""
  );
  const [flightToIata, setFlightToIata] = useState<string>(
    (planItem as any)?.flightDetails?.toIata || ""
  );
  const [flightDepTerminal, setFlightDepTerminal] = useState<string>(
    (planItem as any)?.flightDetails?.depTerminal || ""
  );
  const [flightArrTerminal, setFlightArrTerminal] = useState<string>(
    (planItem as any)?.flightDetails?.arrTerminal || ""
  );

  // ===== ACCOMMODATION DETAILS =====
  const [accomName, setAccomName] = useState<string>(
    (planItem as any)?.accommodationDetails?.name || ""
  );
  const [accomAddress, setAccomAddress] = useState<string>(
    (planItem as any)?.accommodationDetails?.address || ""
  );
  const [accomCity, setAccomCity] = useState<string>(
    (planItem as any)?.accommodationDetails?.city || ""
  );
  const [accomCountry, setAccomCountry] = useState<string>(
    (planItem as any)?.accommodationDetails?.country || ""
  );
  const [accomCheckInAt, setAccomCheckInAt] = useState<Date | null>(
    (planItem as any)?.accommodationDetails?.checkInAt
      ? new Date((planItem as any).accommodationDetails.checkInAt)
      : null
  );
  const [accomCheckOutAt, setAccomCheckOutAt] = useState<Date | null>(
    (planItem as any)?.accommodationDetails?.checkOutAt
      ? new Date((planItem as any).accommodationDetails.checkOutAt)
      : null
  );
  const [accomGuests, setAccomGuests] = useState<number | undefined>(
    (planItem as any)?.accommodationDetails?.guests
  );
  const [accomRooms, setAccomRooms] = useState<number | undefined>(
    (planItem as any)?.accommodationDetails?.rooms
  );
  const [accomBookingRef, setAccomBookingRef] = useState<string>(
    (planItem as any)?.accommodationDetails?.bookingRef || ""
  );
  const [accomPhone, setAccomPhone] = useState<string>(
    (planItem as any)?.accommodationDetails?.phone || ""
  );
  const [accomWebsite, setAccomWebsite] = useState<string>(
    (planItem as any)?.accommodationDetails?.website || ""
  );

  // ===== TRANSPORT DETAILS =====
  type TransportMode = "flight" | "train" | "bus" | "car" | "ferry" | "other";
  const [transportMode, setTransportMode] = useState<TransportMode | null>(
    (planItem as any)?.destinationTransport?.mode || null
  );
  const [transportCompany, setTransportCompany] = useState<string>(
    (planItem as any)?.destinationTransport?.company || ""
  );
  const [transportBookingRef, setTransportBookingRef] = useState<string>(
    (planItem as any)?.destinationTransport?.bookingRef || ""
  );
  const [transportFromName, setTransportFromName] = useState<string>(
    (planItem as any)?.destinationTransport?.fromName || ""
  );
  const [transportToName, setTransportToName] = useState<string>(
    (planItem as any)?.destinationTransport?.toName || ""
  );
  const [transportDepAt, setTransportDepAt] = useState<Date | null>(
    (planItem as any)?.destinationTransport?.depAt
      ? new Date((planItem as any).destinationTransport.depAt)
      : null
  );
  const [transportArrAt, setTransportArrAt] = useState<Date | null>(
    (planItem as any)?.destinationTransport?.arrAt
      ? new Date((planItem as any).destinationTransport.arrAt)
      : null
  );

  // ===== PAYMENT STATUS & EXPENSE CATEGORY =====
  type PaymentStatus = "pending" | "paid" | "refunded";
  const [paymentStatus, setPaymentStatus] = useState<PaymentStatus>(
    (planItem as any)?.paymentStatus || "pending"
  );

  type BudgetCategoryType = "transport" | "accommodation" | "food" | "activities" | "shopping" | "other";
  const [expenseCategory, setExpenseCategory] = useState<BudgetCategoryType | null>(
    (planItem as any)?.expenseDetails?.category || null
  );
  ...
  const handleSave = async () => {
    ...
    if (baseType === "flight") {
      payload.flightDetails = {
        ...
        airlineName: flightAirlineName.trim() || null,
        flightNumberRaw: flightNumber.trim() || null,
        fromIata: flightFromIata.trim() || null,
        toIata: flightToIata.trim() || null,
        depTerminal: flightDepTerminal.trim() || null,
        arrTerminal: flightArrTerminal.trim() || null,
      };
    }

    // Accommodation details
    if (baseType === "accommodation") {
      payload.accommodationDetails = {
        name: accomName.trim() || null,
        address: accomAddress.trim() || null,
        city: accomCity.trim() || null,
        country: accomCountry.trim() || null,
        checkInAt: accomCheckInAt?.toISOString() || null,
        checkOutAt: accomCheckOutAt?.toISOString() || null,
        guests: accomGuests || null,
        rooms: accomRooms || null,
        bookingRef: accomBookingRef.trim() || null,
        phone: accomPhone.trim() || null,
        website: accomWebsite.trim() || null,
      };
    }

    // Transport details
    if (baseType === "transport") {
      payload.destinationTransport = {
        mode: transportMode,
        company: transportCompany.trim() || null,
        bookingRef: transportBookingRef.trim() || null,
        fromName: transportFromName.trim() || null,
        toName: transportToName.trim() || null,
        depAt: transportDepAt?.toISOString() || null,
        arrAt: transportArrAt?.toISOString() || null,
      };
    }
    ...
    // Expense category (only for non-logistics items)
    if (expenseCategory && !["flight", "accommodation", "transport"].includes(baseType)) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      >
        ...
        <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
          ...
          >
            <View className="flex-row items-center">
              ...
              <Text
                className={`text-[12px] ${selectedTransaction ? "text-indigo-900" : "text-indigo-700"
                  }`}
              >
                {selectedTransaction
        ...
        {baseType === "flight" && (
          <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
            <FlightDetailsFields
              ...
              fromIata={flightFromIata}
              toIata={flightToIata}
              depTerminal={flightDepTerminal}
              arrTerminal={flightArrTerminal}
              onChangeProvider={setFlightProvider}
              onChangeAirlineName={setFlightAirlineName}
              onChangeFlightNumber={setFlightNumber}
              onChangeFromIata={setFlightFromIata}
              onChangeToIata={setFlightToIata}
              onChangeDepTerminal={setFlightDepTerminal}
              onChangeArrTerminal={setFlightArrTerminal}
            />
          </View>
        )}

        {/* EXTENDED DETAILS - Accommodation */}
        {baseType === "accommodation" && (
          <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
            <AccommodationDetailsFields
              name={accomName}
              address={accomAddress}
              city={accomCity}
              country={accomCountry}
              checkInAt={accomCheckInAt}
              checkOutAt={accomCheckOutAt}
              guests={accomGuests}
              rooms={accomRooms}
              bookingRef={accomBookingRef}
              phone={accomPhone}
              website={accomWebsite}
              onChangeName={setAccomName}
              onChangeAddress={setAccomAddress}
              onChangeCity={setAccomCity}
              onChangeCountry={setAccomCountry}
              onChangeCheckInAt={setAccomCheckInAt}
              onChangeCheckOutAt={setAccomCheckOutAt}
              onChangeGuests={(val) => setAccomGuests(val ? parseInt(val) : undefined)}
              onChangeRooms={(val) => setAccomRooms(val ? parseInt(val) : undefined)}
              onChangeBookingRef={setAccomBookingRef}
              onChangePhone={setAccomPhone}
              onChangeWebsite={setAccomWebsite}
            />
          </View>
        )}

        {/* EXTENDED DETAILS - Transport */}
        {baseType === "transport" && (
          <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
            <TransportDetailsFields
              mode={transportMode}
              company={transportCompany}
              bookingRef={transportBookingRef}
              fromName={transportFromName}
              toName={transportToName}
              depAt={transportDepAt}
              arrAt={transportArrAt}
              onChangeMode={setTransportMode}
              onChangeCompany={setTransportCompany}
              onChangeBookingRef={setTransportBookingRef}
              onChangeFromName={setTransportFromName}
              onChangeToName={setTransportToName}
              onChangeDepAt={setTransportDepAt}
              onChangeArrAt={setTransportArrAt}
            />
          </View>
        )}

        {/* PAYMENT STATUS - show if there's a cost */}
        {cost && (
          <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
            <Text className="text-[13px] font-semibold text-gray-900 mb-2">
              Estado de pago
            </Text>
              ...
              {[
                ...
                return (
                  <TouchableOpacity
                    ...
                    className="flex-1 rounded-2xl py-2.5 border"
                    style={{
                      borderColor: active ? status.color : "rgba(148,163,184,0.22)",
                      backgroundColor: active ? `${status.color}15` : "white",
                    }}
                    activeOpacity={0.9}
                  >
                    <Text
                      className="text-center text-[12px] font-bold"
                      style={{ color: active ? status.color : "#0B1220" }}
                    >
                      {status.label}
                    </Text>
                  </TouchableOpacity>
        ...
        {/* EXPENSE CATEGORY - only for non-logistics items with cost */}
        {!(["flight", "accommodation", "transport"] as string[]).includes(baseType) && cost && (
          <View className="rounded-3xl bg-white p-4 shadow-sm mb-2">
            <Text className="text-[13px] font-semibold text-gray-900 mb-2">
              Categoría de gasto
            </Text>
            <View className="flex-row flex-wrap gap-2">
              {[
                ...
                const active = expenseCategory === cat.value;
                return (
                  <TouchableOpacity
                    key={cat.value}
                    onPress={() => setExpenseCategory(cat.value as any)}
                    className="rounded-2xl px-3 py-2 border"
                    style={{
                      borderColor: active ? "#2563EB" : "rgba(148,163,184,0.22)",
                  ...
                  >
                    <Text
                      ...
                      style={{ color: active ? "#2563EB" : "#0B1220" }}
                    >
                      {cat.label}
                    </Text>
                  </TouchableOpacity>
